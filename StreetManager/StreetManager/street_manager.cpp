...
StreetManager::StreetManager(QWidget *parent)
    : QMainWindow(parent)
...
bool StreetManager::isAddressUnique(const QString &addr)
{
...
void StreetManager::addHousehold(QString& address, QString& headId, QVector<QString> memberIds, int type, int specialStatus)
{
    ...
    auto numMembers = memberIds.size();
    if (numMembers)
    {
        for (int i = 0; i < numMembers; ++i)
        {
            do
            {
                if (heads.find(memberIds[i]) != heads.end())
                {
                    // cout << "Loi: ID thanh vien " << memberIds[i] << " da ton tai! Vui long nhap lai!\n";
                }
                else if (familyMembers.find(memberIds[i]) != familyMembers.end())
                {
                    FamilyMember fm = familyMembers.value(memberIds[i]);
                    fm.inputHeadIdAndRelationship(h->getHeadId());
                    break;
                }
                else if (familyMembers.find(memberIds[i]) == familyMembers.end())
                {
                    cout << "ID thanh vien " << memberIds[i] << " khong ton tai! Vui long them thanh vien truoc.\n";
                    this->addMemberWithOutId(memberIds[i], headId);
                    break;
                }
                else if (familyMembers[memberIds[i]].getHeadId() != headId)
                {
                    cout << "Loi: ID chu ho cua thanh vien (" << familyMembers[memberIds[i]].getHeadId() << ") khong khop voi chu ho (" << headId << "). Vui long nhap lai!\n";
                }
            } while (true);

            memberIds.push_back(memberIds[i]);
        }
    }
    ...
    households.push_back(std::move(h));
