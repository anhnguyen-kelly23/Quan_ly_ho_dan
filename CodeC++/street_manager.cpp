...
bool StreetManager::isAddressUnique(const string &addr) const //Kiem tra  dia chi ho dan co ton tai hay khong   
{
...
const HouseholdHead *StreetManager::getHead(const string &id) const //Kiem tra ID chu ho co ton tai hay khong
{
  auto it = heads.find(id);
  return it != heads.end() ? &it->second : nullptr;
}

const FamilyMember *StreetManager::getMember(const string &id) const //Kiem tra ID thanh vien co ton tai hay khong
{
  auto it = familyMembers.find(id);
  return it != familyMembers.end() ? &it->second : nullptr;
}

void StreetManager::addHousehold() //Them ho dan
{
  auto type = getValidIntegerInput("Chon loai ho (1: Thuong tru, 2: Tam tru): ", 1, 2);
  if (!type)
    return;

  unique_ptr<Household> h; //Khoi tao con tro ho dan
  ...
  HouseholdHead head; //Khoi tao chu ho
  ...
  else if (familyMembers.find(headId) != familyMembers.end()) //Kiem tra ID chu ho co ton tai trong danh sach thanh vien
  {
  ...
  SpecialStatus status = SpecialStatus::None; //Khoi tao trang thai dac biet
  cout << "Nhap trang thai dac biet:\n";
  cout << "0. Khong co\n1. Can ngheo\n2. Ngheo\n";
  auto statusChoice = getValidIntegerInput("Nhap lua chon: ", 0, 2); //Nhap trang thai dac biet
  if (statusChoice)
  {
    switch (*statusChoice)
    {
  ...
  if (numMembers)
  {
    for (int i = 0; i < *numMembers; ++i)
    {
      ...
      do
      {
        memId = getValidStringInput("Nhap ID (9 hoac 12 chu so): ", 12, false); //Nhap ID thanh vien
        ...
        else if (familyMembers.find(memId) == familyMembers.end()) //Kiem tra ID thanh vien co ton tai trong danh sach thanh vien
        {
          cout << "ID thanh vien " << memId << " khong ton tai! Vui long them thanh vien truoc.\n";
          this->addMemberWithOutId(memId, headId);
          break;
        }
        else if (familyMembers[memId].getHeadId() != headId) //Kiem tra ID chu ho cua thanh vien co khop voi ID chu ho
        {
  ...
  if (*type == 1)
  {
    h = make_unique<PermanentHousehold>(addr, headId, memberIds, status); //Khoi tao ho thuong tru
  }
  else
  {
    Date expiryDate; //Khoi tao ngay het han
    do
    {
      string expStr = getValidStringInput("Nhap ngay het han tam tru (dd/mm/yyyy, sau " +
                                              Date::currentDate().toString() + ", vi du: 01/12/2023): ",
                                          10, false);
      string errorMsg; //Khoi tao chuoi loi
      expiryDate = Date::fromString(expStr, errorMsg);
      if (!expiryDate.isValid())
      {
        cout << "Loi: " << errorMsg << " Vui long nhap lai.\n";
      }
      else if (!expiryDate.isFuture()) //Kiem tra ngay het han co phai sau ngay hien tai hay khong
    ...
    h = make_unique<TemporaryHousehold>(addr, headId, memberIds, status, expiryDate); //Khoi tao ho tam tru
  households.push_back(move(h));
  cout << "Them ho dan thanh cong!\n";
}
}

void StreetManager::editHousehold() //Chinh sua ho dan
{
  string searchTerm = getValidStringInput("Nhap dia chi hoac ID chu ho de tim kiem: ", 100, false); //Nhap dia chi hoac ID chu ho
  if (searchTerm.empty())
    return;
  for (auto &h : households)
  {
    if (h->getAddress() == searchTerm || h->getHeadId() == searchTerm) //Kiem tra dia chi ho dan co ton tai hay khong
    {
...
void StreetManager::deleteHousehold() //Xoa ho dan
{
  string searchTerm = getValidStringInput("Nhap dia chi hoac ID chu ho de xoa: ", 100, false); //Nhap dia chi hoac ID chu ho
  if (searchTerm.empty())
    return;
  for (auto it = households.begin(); it != households.end(); ++it) //Duyet qua danh sach ho dan
  {
...
void StreetManager::manageHouseholdMembers() //Quan ly thanh vien ho
{
  ...
  for (auto &h : households)
  {
    if (h->getAddress() == searchTerm || h->getHeadId() == searchTerm) //Kiem tra dia chi ho dan co ton tai hay khong
    {
      while (true)
      {
        ...
        cout << "0. Quay lai\n";
        auto choice = getValidIntegerInput("Nhap lua chon: ", 0, 2); //Nhap lua chon
        if (!choice || *choice == 0)
          break;
        switch (*choice)
        {
        case 1:
        {
          ...
          auto &memIds = const_cast<vector<string> &>(h->getMemberIds()); //Lay danh sach ID thanh vien
...
void StreetManager::addMemberWithOutId(string id, string headId) //Them thanh vien ho khong co ID
{
  FamilyMember fm;
...
void StreetManager::managePersons() //Quan ly nguoi dan
{
  while (true)
  {
    cout << "\nQUAN LY NGUOI DAN\n";
...
void StreetManager::addPerson() //Them nguoi dan
{
  FamilyMember fm;
...
void StreetManager::findExpiredTemporaryHouseholds() //Tim kiem ho tam tru het han
{
  bool found = false;
...
void StreetManager::displayAll() const //Hien thi tat ca ho dan
{
  if (households.empty())
  {
...
void StreetManager::searchByAddress(const string &addr) const // Tim kiem ho dan theo dia chi
{
  bool found = false;
  for (const auto &h : households)
  {
...
void StreetManager::searchById(const string &id) const //Tim kiem ho dan theo ID
{
  bool found = false;
  for (const auto &h : households)
  {
...
void StreetManager::reportByType() const //Thong ke ho dan theo loai
{
...
bool StreetManager::loadFromCsv() //Doc du lieu tu file CSV
{
  households.clear();
  heads.clear();
  familyMembers.clear();
...

  {
    ...
    while (getline(file, line))
    {
      ...
      vector<string> memberIds; //Khoi tao danh sach ID thanh vien
